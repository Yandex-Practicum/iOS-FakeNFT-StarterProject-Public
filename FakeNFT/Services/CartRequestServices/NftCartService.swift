...
class NftCartService {
    ...
    func getAllNFTs() async throws -> [Nft] {
        let data = try await networkService.performRequest(endpoint: "api/v1/nft?page=1&size=10")
        return try JSONDecoder().decode([Nft].self, from: data)
    }
    
    func getNFT(id: String) async throws -> Nft {
        let data = try await networkService.performRequest(endpoint: "/api/v1/nft/\(id)")
        return try JSONDecoder().decode(Nft.self, from: data)
    }
}
